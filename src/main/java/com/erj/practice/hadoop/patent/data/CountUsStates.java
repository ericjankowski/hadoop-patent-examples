...
public class CountUsStates extends Configured implements Tool{
	...
	public static class ImTheMap extends MapReduceBase implements Mapper<Text, Text, Text, IntWritable> {
		public void map(Text key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
			//Parse the line and make a tick mark for each US state 
			String line = key.toString();
			line = line.replaceAll("\"", "");
			String [] components = line.split(",");
			try{
				Integer.parseInt(components[0]);
				if(components[4].equals("US") && components[5].length()>0){
					output.collect(new Text(components[5]), one);
				}
	...
	public int run (String [] args) throws Exception {
		...
		JobConf job = new JobConf(conf, CountUsStates.class);
		Path in = new Path(args[0]);
		Path out = new Path(args[1]);
		FileInputFormat.setInputPaths(job, in);
		FileOutputFormat.setOutputPath(job, out);
		
		job.setJobName("StateCount");
	...
	public static void main (String [] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),  new CountUsStates(), args);
		System.exit(res);
	}
}
